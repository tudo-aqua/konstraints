...
class PushTests {
  ...
  fun simpleGDartDSEExample() {
    ...
    val nullConst = program.declareConst("null", SMTInt)
                  ...
                  not(
                      And(
                          ...
                          objExtends.constructDynamic(
                              listOf(obj0cls, StringLiteral("LB;")),
                              emptyList(),
                          ),
                      )
                  )
    ...
    program
        .push(getSolver(), true) {
          ...
          assert(
              And(
                  not(obj0 eq nullConst),
                  objExtends.constructDynamic(listOf(obj0cls, StringLiteral("LB;")), emptyList()),
              )
          )
          assert(
              not(
                  And(
                      not(obj0 eq nullConst),
                      objExtends.constructDynamic(
                          listOf(obj0cls, StringLiteral("LA;")),
                          emptyList(),
                      ),
                  )
              )
    ...
    program
        .push(getSolver(), true) {
          ...
          assert(
              And(
                  not(obj0 eq nullConst),
                  objExtends.constructDynamic(listOf(obj0cls, StringLiteral("LA;")), emptyList()),
              )
          )
          assert(
              not(objExtends.constructDynamic(listOf(obj0cls, StringLiteral("LC;")), emptyList()))
          )
        }
        .let { (status, _) -> assertEquals(SatStatus.SAT, status) }
    ...
    // push 7: extends LC, not-null, NOT method.of (UNSAT: only LC satisfies, but method.of LC is
    // true)
    program
        .push(getSolver(), true) {
          assert(obj0err eq "")
