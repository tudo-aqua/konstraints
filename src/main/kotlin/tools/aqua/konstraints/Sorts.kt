...
/**
 * Data class representing an tools.aqua.konstraints.SMTSymbol constructor checks if [symbol] is a
 * valid simple symbol or quoted symbol quotes symbol if needed, allows for explicit and implicit
 * creation of quoted symbols
 *
...
data class SMTSymbol(val symbol: String) {
  ...
  /**
   * Companion object to store [Regex] so they don't need to be created everytime a Symbol is
   * created
   */
  ...
  /**
   * Check if [symbol] is a valid SMT Symbol by matching it first with a simple symbol second with a
   * quoted symbol third with a list of reserved words (TODO)
   *
   * @throws IllegalArgumentException if [symbol] is not valid
   */
  init {
...
class SMTIdentifier(symbol: SMTSymbol, varargs: List<String>) {
  ...
  private val symbol: String =
      if (varargs.isEmpty()) symbol.toString()
...
open class Sort(val identifier: SMTIdentifier, val varargs: List<Sort>) {
  ...
  override fun toString() =
      if (varargs.isEmpty()) "$identifier"
      else "($identifier ${varargs.joinToString(separator = " ")})"
}
 ...
 */
class BVSort private constructor(val bits: Int) :
    Sort(SMTIdentifier(SMTSymbol("BitVec"), listOf("$bits"))) {
  companion object {
...
object ExampleListSort :
    Sort(
        SMTIdentifier(SMTSymbol("List")),
        listOf(Sort(SMTIdentifier(SMTSymbol("Array")), listOf(IntSort, RealSort))))

object ExampleFixedSizeList :
    Sort(SMTIdentifier(SMTSymbol("FixedSizeList"), listOf("4")), listOf(RealSort))
...
open class IndexedSort(val name: String, val arity: List<Int>) : Sort(TODO()) {
  override fun equals(other: Any?): Boolean =
      when {
        this === other -> true
        other !is IndexedSort -> false
        else -> name == other.name && arity == other.arity
      }

  override fun hashCode(): Int = name.hashCode() + 31 * arity.hashCode()
